...
def browse():
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('select id, date, title, content from data order by date')
    rowlist = cursor.fetchall()
    print(rowlist);
    return render_template('browse.html', data=rowlist)
...
def main():
    ...
    surname = "Sambor"
    group = "KID-21"
    return render_template("index.html", name=name, surname=surname, group=group)
